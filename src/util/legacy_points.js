...
export const isTooHighToPlay = (c, pegCount) => valueMaxTen(c) > (31 - pegCount);
...
export function isFifteen(...cards) {
  const sum = cards.map(card => valueMaxTen(card)).reduce((acc, curr) => (acc + curr), 0);
  return sum === 15;
}
...
export function getFifteens(hand) {
  const fifteens = {
    1: [], 2: [], 3: [], 4: [], 5: [],
  }; // index = number of cards in combos
...
export function sumLengths(obj) {
  return Object.keys(obj).map(k => obj[k].length).reduce((acc, curr) => acc + curr, 0);
}

// Sum all values in obj
export function sumValues(obj) {
  return Object.keys(obj).map(k => obj[k]).reduce((acc, curr) => acc + curr, 0);
}
